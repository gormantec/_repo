...
{
    Debug.log("[findElementByPointer:start] count="+this._elements.length.toString());
    ...
    if(result)Debug.log("[findElementByPointer:end] "+result.toString());
    return result;
}
...
static fromPointer(p:i32):Element
{
    ...
    if(e1){
        ...
        var s:string=jsdom.nodeName(p).toString();
        if(p==0){
