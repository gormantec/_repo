...
class PWA {
    ...
    show(win: Window): void {
        ...
        var rootWindow: Element = win.document.body;
        var aPWA = this;
        //if( mockFrame && !( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) ) {
        //    (async () => {try{rootWindow=(await import('/modules/'+mockFrame+'.mjs')).addFrame(win,aPWA, mockFrame);}catch(e){this.debug.log(e);}})();
        //}
        ...
        if (!win.PWA.globals.splashColor) rootWindow.style.backgroundColor = this.primaryColor;
        if (!win.PWA.globals.splashColor) rootWindow.style.color = this.primaryColorText;
        ...
        this.addStyle(win.document, "https://fonts.googleapis.com/icon?family=Material+Icons|Material+Icons+Outlined|Material+Icons+Two+Tone|Material+Icons+Round|Material+Icons+Sharp", function () {
            ...
            Debug.log("[show:8]");
                ...
                (<PWA>_thisPWA).addStyle(Window.window.document, "https://gcode.com.au/css/pwa.css", function () {
                    ...
                    setTimeout(function () {
                        Debug.log("[show:10]");
                        while (Window.window.document.body.firstChild) Window.window.document.body.removeChild(Window.window.document.body.lastChild);
                        ...
                        Debug.log("SHOW");
                        Window.window.document.body.appendChild((<PWA>_thisPWA).pwaRoot.element);
                        Window.window.document.body.appendChild((<PWA>_thisPWA).pwaOverlay.element);
